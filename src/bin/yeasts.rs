...
fn main() {
    for yeast in Yeast::iter() {
        println!("{}", yeast);
        println!("  TEMP {}..{}", yeast.temp_range().start, yeast.temp_range().end);
        println!("  ATTENUATION {}..{}", yeast.attenuation_range().start, yeast.attenuation_range().end);
        println!("  ALCOHOL TOLERANCE {}..{}", yeast.alcohol_tolerance_range().start, yeast.alcohol_tolerance_range().end);
